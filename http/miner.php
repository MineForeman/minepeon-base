...
$mobilepage        = array(
    ...
    'DEVS+NOTIFY' => array(
        ...
        'DEVS.MHS av=MHS av',
        'DEVS.Accepted=Accept',
        'DEVS.Rejected=Rej',
        'DEVS.Utility=Utility',
...
function getsock($rig, $addr, $port)
{
    ...
    socket_set_option($socket, SOL_SOCKET, SO_SNDTIMEO, array(
        'sec' => $socksndtimeoutsec,
        'usec' => 0
    ));
    socket_set_option($socket, SOL_SOCKET, SO_RCVTIMEO, array(
        'sec' => $sockrcvtimeoutsec,
        'usec' => 0
    ));
    ...
    if ($value === null)
        ...
        switch ($section . '.' . $name) {
            ...
            case 'total.Utility':
                ...
                if ($value == 0)
                    $class = $errorclass;
                else if (isset($alldata['Difficulty Accepted']) && isset($alldata['Accepted']) && isset($alldata['MHS av']) && ($alldata['Difficulty Accepted'] > 0) && ($alldata['Accepted'] > 0)) {
            ...
            case 'DEVS.Fan Speed':
                if ($value == 0)
                    $class = $warnclass;
                else if (isset($alldata['Fan Percent'])) {
            ...
            case 'total.MHS av':
                ...
                if ($value == 0)
                    $class = $errorclass;
                else if (isset($alldata['Difficulty Accepted']) && isset($alldata['Accepted']) && isset($alldata['Utility']) && ($alldata['Difficulty Accepted'] > 0) && ($alldata['Accepted'] > 0)) {
...
function secmatch($section, $field)
{
    ...
    if ($section == 'DEVS' && ($field == 'GPU' || $field == 'PGA'))
        return true;
...
function checklogin()
{
    ...
    if (($rut !== null && $rut !== '') || ($roh !== null && $roh !== '')) {
        ...
        if ($rut !== null && $rut !== '' && $roh !== null && $roh !== '') {
            if (isset($userlist['sys']) && isset($userlist['sys'][$rut]) && ($userlist['sys'][$rut] === $roh)) {
                $_SESSION[$ses] = true;
                return false;
            }
            
            if (isset($userlist['usr']) && isset($userlist['usr'][$rut]) && ($userlist['usr'][$rut] === $roh)) {
                $_SESSION[$ses] = false;
                $readonly       = true;
                return false;
            }
...
function display()
{
    ...
    otherrow('<td><br><br></td>');
    newtable();
    doforeach('devs', 'device list', $sum, array(
        '' => '',
        'ID' => '',
        'Name' => ''
    ), false);
